...
class Andor:
    def __init__(self):
        ...
        self.dll = WinDLL("C:\\Program Files\\Andor Driver Pack 2\\atmcd64d")
        self.dll.Initialize("")
    ...
    def StartAcquisition(self):
        ...
        #while (self.dll.GetStatus()!=20002):
        #    time.sleep(0.002)
    ...
    def GetAcquiredData(self,imageArray):
        ...
        print "len(imageArray) before = %d" % len(imageArray)
        print "len(cimage) = %d, dim = %d, len(imageArray) = %d" % (len(cimage), dim, len(imageArray))
        if (self.AcquisitionMode==1):
            #for i in range(len(cimage)):
            #    imageArray.append(cimage[i])
            #imageArray = [cimage[i] for i in range(len(cimage))]
            imageArray = imageArray + [cimage[i] for i in range(len(cimage))]
        else:
            for i in range(len(cimage)):
                imageArray.append(cimage[i])

        endTime = default_timer()    
        print "Andor acq time = %.3f" % (endTime - startTime)
    ...
    def GetAcquiredData2(self,imageArrayPointer):      
        dim = self.GetAcquiredDataDim()

       # startTime = default_timer()                                       

        error = self.dll.GetAcquiredData(imageArrayPointer,dim)
        #self.verbose(ERROR_CODE[error], sys._getframe().f_code.co_name)                   
    ...
    def GetAcquiredDataDim(self):
        w = self.vend - self.vstart + 1
        h = self.hend - self.hstart + 1
        ...
        if (self.ReadMode==4):
            if (self.AcquisitionMode==1):
                dim = w * h / self.hbin / self.vbin
            elif (self.AcquisitionMode==3):
                dim = w * h / self.hbin / self.vbin * self.scans
        elif (self.ReadMode==3 or self.ReadMode==0):
            if (self.AcquisitionMode==1):
                dim = w
            elif (self.AcquisitionMode==3):
    ...
    def SetSingleScan(self):
        ...
        self.SetImage(1,8,1,self.width,1,self.height)
    ...
    def SetDemoReady(self):
        ...
        error = self.SetShutter(1,1,0,0)
        error = self.SetExposureTime(.3)
        return error
    ...
    def SetBinning(self,binningmode):
        if (binningmode==1):
            ...
            self.SetImage(8,8,1,self.width,1,self.height)
        else:
