...
class SessionData(QtCore.QObject):
	...
	# 	self.name = "Fake Session 1"
	# 	self.timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")

	# 	fakeExp = ExperimentData('exp A')
	# 	fakeExp.generateTestData()
	# 	self.addExperiment(fakeExp)

	# 	fakeExp = ExperimentData('exp B')
	# 	fakeExp.generateTestData()
	# 	self.addExperiment(fakeExp)
...
class ExperimentData:
	...
	# returns a list of numpy.array=([x,y])
	# All in cartesian coords
	def getMeanScanCoords(self, laserPos):
		coordList = np.empty((0,2), float)
		for scan in self.scanList:
			if scan.deleted:
				continue
			pupilPos = np.array([scan.MeanPupilCenters[0], scan.MeanPupilCenters[1]])
			measPos = laserPos - pupilPos
			coordList = np.vstack((coordList, measPos))
		return coordList
...
class ScanData:
	...
	# Attributes to save
	# TODO: set the types of these attributes
	savedAttributes = ['timestamp', 'note', 'temperature', 'deleted']	# these are hdf5 attributes
	...
	savedDataset = ['CMOSImage',
					...
					'RawTempList',
					'CalTempList',
					'BS',			# a single number computed from BSList
					'SD',
					'FSR',
